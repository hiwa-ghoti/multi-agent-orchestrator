...
```bash
npm run ui:open
# Windows PowerShell なら npm.cmd run ui:open
```

**いちばん簡単（Windows）:** エクスプローラーで `start-ui.cmd` をダブルクリック  
→ サーバー起動 + ブラウザが自動で開きます。
...
ブラウザ: [http://127.0.0.1:3847](http://127.0.0.1:3847)

- **once / parallel / loop** をタブで切り替え
- 実行ログが画面右側に流れる
- **停止** ボタンで中断できる
- API キーはブラウザに送らず、サーバーが `.env` を読む
- **localhost 専用**（外部公開しない）
...
1. ~~Phase 2 — README / デモを整え、GitHub に公開~~（ログ例・図・ローカル GUI まで）
